...
| review-and-commit    | adapt    | Originally depended on a `commit-full` skill in pi-setup that no longer exists. Migrated self-contained and aligned with the plan/execute split of the `commit-plan` skill: reviewer (read-only) audits, the parent builds an approved commit plan, then the worker agent executes it (fixes, cleanup, tests, local commits). No pi-setup runtime dependency. |
